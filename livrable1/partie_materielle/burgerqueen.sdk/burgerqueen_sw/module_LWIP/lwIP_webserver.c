                               ...
                               struct pbuf *p, err_t err)
{
	...
	if(rcv_b.body_size == 0 && rcv_b.header_size == 0) {
		...
		char *ptr = strstr(p->payload, "Content-Length"); // + 16 pour aller chercher le nombre
		char *size_body = strtok(ptr + 16, (char*)' ');

		// Si size_body n'est pas NULL, ca veut dire que l'on a un payload
		if (size_body != NULL) {
			rcv_b.body_size = atoi(size_body);
			// On va chercher le size du header au complet
			ptr = strstr(p->payload, "{");
			rcv_b.header_size = (int)((void*)ptr - (void*)p->payload);
		}
		// Si size_body est null, cela veut dire que le paquet n'a pas de body
		else {
			rcv_b.body_size = 0;
...
err_t accept_callback(void *arg, struct tcp_pcb *newpcb, err_t err)
{
	...
	//xil_printf("On a recu une requete!!!!!!!!\n\r");
	tcp_recv(newpcb, recv_callback);
	tcp_sent(newpcb, sent_callback);
	//struct receive_bucket *rcv_b = (struct receive_bucket*)malloc(sizeof(struct receive_bucket));
	rcv_b.header_size = 0;
	rcv_b.body_size = 0;
	rcv_b.curr_data_pos = 0;
	tcp_arg(newpcb, NULL/*(void*)rcv_b*/);
...
void init_netif()
{
	...
	{ 0x00, 0x18, 0x3e, 0x00, 0x00, 0x26 };


	/* initliaze IP addresses to be used */
	IP4_ADDR(&ipaddr,  132, 207,   89, 26);
