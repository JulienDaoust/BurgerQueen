...
FATFS FatFs;
UCHAR fontTexture[1048576];
UCHAR Burger[4000000];
	...
	if (me->photo_client[0] != '\0') {
		...
		for(i=YOFF; i<64+YOFF; ++i) {
			for(j=XOFF; j<64+XOFF; ++j) {
				AFF_BackBuffer[i*SCRN_WIDTH + j ] = *(int*)(me->photo_client + 54 + ((64-1-(i-YOFF))*64 + (j-XOFF))*3);
			}
		}
	}
...
AFF_PrintDetailCmd(struct CMD_CmdList* cmdlist) {
	...
	AFF_PrintString(cmd->no_table, strlen(cmd->no_table), 50 + CHAR_HEIGHT*3, 50 + CHAR_WIDTH*17);
	/****************************************************************/
	txt = "COURRIEL: ";
